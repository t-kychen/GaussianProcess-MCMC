...
class DataSet(object):
    ...
    def getData(self, file_district, file_year):
        ...
        self.data = np.genfromtxt(file_name, delimiter=",", dtype="f8", usecols=range(1,31), skiprows=1)
    ...
    def getRoute(self, target_route):
        ...
        #route_idx = self.getRoute(target_route)
        if len(route_idx) == 0:
            raise Exception("Selected route not found!")
        else:
            # extract only the data on the selected route
